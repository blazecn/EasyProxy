...
pub fn send_message(stream: &mut UnixStream, msg: &IpcMessage) -> Result<(), String> {
    ...
    stream
        ...
        .map_err(|e| format!("写入 IPC 长度失败: {e}"))?;
    stream
        .write_all(&json)
        .map_err(|e| format!("写入 IPC 消息失败: {e}"))?;
    stream
        .flush()
        .map_err(|e| format!("刷新 IPC 流失败: {e}"))?;
    Ok(())
}
...
pub fn recv_message(stream: &mut UnixStream) -> Result<IpcMessage, String> {
    ...
    stream
        ...
        .map_err(|e| format!("读取 IPC 长度失败: {e}"))?;
    ...
    stream
        .read_exact(&mut buf)
        .map_err(|e| format!("读取 IPC 消息数据失败: {e}"))?;
    let msg: IpcMessage =
        serde_json::from_slice(&buf).map_err(|e| format!("解析 IPC 消息失败: {e}"))?;
    if !msg.verify() {
